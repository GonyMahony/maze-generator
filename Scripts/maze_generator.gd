...
func _draw() -> void:
	...
	for x in range(size_x):
		for y in range(size_y):
			...
			if n.has_wall("n"):
				draw_line(
					Vector2(x * block_size_x, y * block_size_y),
					Vector2((x + 1) * block_size_x, y * block_size_y),
					Color.BLACK, 2
				)
			...
			if n.has_wall("w"):
				draw_line(
					Vector2(x * block_size_x, y * block_size_y),
					Vector2(x * block_size_x, (y + 1) * block_size_y),
					Color.BLACK, 2
				)
			...
			if n.has_wall("e"):
				draw_line(
					Vector2((x + 1) * block_size_x, y * block_size_y),
					Vector2((x + 1) * block_size_x, (y + 1) * block_size_y),
					Color.BLACK, 2
				)
	...
	for i in range(path.size() - 1):
		...
		var p2 = path[i + 1]
		draw_line(
			Vector2(p1.col * block_size_x + block_size_x / 2, p1.row * block_size_y + block_size_y / 2),
			Vector2(p2.col * block_size_x + block_size_x / 2, p2.row * block_size_y + block_size_y / 2),
			Color.RED, 2
		)

	# Step generator
	if visited < size_x * size_y and steps > 0 and cell != null:
		steps -= 1
		draw_circle(
			Vector2(cell.col * block_size_x + block_size_x / 2,
			cell.row * block_size_y + block_size_y / 2),
			block_size_x / 2,
			Color.BLACK
		)
		next_step()
